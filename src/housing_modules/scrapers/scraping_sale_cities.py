...
def get_all_webpages(limit, citta):
    urls = []
    urls.append(f"https://www.immobiliare.it/vendita-case/{citta}/?criterio=rilevanza")
    for page in range(2, limit):
        url = f"https://www.immobiliare.it/vendita-case/{citta}/?criterio=rilevanza&pag={page}"
        urls.append(url)
    return urls
